...
(function () {
  ...
  const bottomNavItems = document.querySelectorAll('.bn-item[data-view]');
  const bottomMoreBtn = document.querySelector('[data-bottom-more]');
  // Views that live only inside the "More" drawer (no dedicated bottom-nav tab)
  const moreViews = ['repairs', 'sales', 'messages', 'settings'];
  ...
  function openSidebar() {
    ...
    bottomMoreBtn && bottomMoreBtn.setAttribute('aria-expanded', 'true');
  ...
  function closeSidebar() {
    ...
    bottomMoreBtn && bottomMoreBtn.setAttribute('aria-expanded', 'false');
    document.body.style.overflow = '';
  }
  menuToggle && menuToggle.addEventListener('click', openSidebar);
  bottomMoreBtn && bottomMoreBtn.addEventListener('click', openSidebar);
  ...
  function show(name) {
    ...
    // Keep bottom nav in sync. Views without a dedicated tab (Repairs, Sales,
    // Messages, Settings) light up "More" instead, since that's where they live.
    bottomNavItems.forEach(n => n.classList.toggle('active', n.dataset.view === name));
    if (bottomMoreBtn) bottomMoreBtn.classList.toggle('active', moreViews.includes(name));
  ...
  bottomNavItems.forEach(n => n.addEventListener('click', e => {
    e.preventDefault();
    show(n.dataset.view);
    history.replaceState(null, '', '#' + n.dataset.view);
    closeSidebar();
